...
export const Filters = ({ setPokemonsFilters, setPokemons, setPage }) => {
    const
        [selectTypeFilter, setSelectTypeFilter] = useState(''),
        [selectAbilityFilter, setSelectAbilityFilter] = useState('')

    useEffect(() => {
        const
            fetchPokemonsType = async () => {
                const
                    response = await fetch(`${url}/type/${selectTypeFilter}`),
                    data = await response.json()

                const
                    filter = data.pokemon
                        .filter(data => data.pokemon.url.includes(data.pokemon.url.split("/")[6]) && data.pokemon.url.split("/")[6] <= 649)
    ...
    useEffect(() => {
        const
            fetchPokemonsAbility = async () => {
                const
                    response = await fetch(`${url}/ability/${selectAbilityFilter}`),
                    data = await response.json()
